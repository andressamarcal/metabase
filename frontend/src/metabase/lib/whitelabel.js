...
export const colorForScheme = (scheme, original) =>
    original
        ...
        .saturationl(scheme.saturationl() *
            (1 + (original.saturationl() - BRAND_NORMAL_COLOR.saturationl()) / BRAND_NORMAL_COLOR.saturationl()))
        .lightness(scheme.lightness() *
            (1 + (original.lightness() - BRAND_NORMAL_COLOR.lightness()) / BRAND_NORMAL_COLOR.lightness()))
        .string();
...
const replaceBrandColors = (value, colorScheme) => {
    ...
    return value.replace(COLOR_REGEX, (colorOriginal) => {
        ...
        } else if (hasSimilarHue(BRAND_NORMAL_COLOR, original, DESATURATE_THRESHOLD)) {
...
function updateHarmoneyColors(colorScheme) {
    ...
    const closest = findClosestIndex(
        HARMONY_ORIGINAL.map(c => Color(c).hsl().hue()),
    ...
    const newHarmony = [colorScheme, ...HARMONY_ORIGINAL.slice(closest), ...HARMONY_ORIGINAL.slice(0, closest)];
